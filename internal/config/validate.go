...
func Validate(cfg ProxyConfig) error {
	...
	for _, c := range cfg.Clusters {
		...
		if _, dup := clusters[c.Name]; dup {
			return fmt.Errorf("cluster: duplicate name %q", c.Name)
		}
		clusters[c.Name] = c
	...
	for i, r := range cfg.Routes {
		if r.Prefix == "" {
			return fmt.Errorf("route[%d]: empty prefix", i)
		}
		if r.Cluster == "" {
			return fmt.Errorf("route[%d]: empty cluster", i)
		}
	...
	for _, l := range cfg.Listeners {
		if l.Name == "" {
			return fmt.Errorf("listener: empty name")
		}
		if l.Address == "" {
		...
		if l.TLS != nil {
			if err := fileExists(l.TLS.CertPath); err != nil {
				return fmt.Errorf("listener %q: tls cert: %w", l.Name, err)
			}
			if err := fileExists(l.TLS.KeyPath); err != nil {
				return fmt.Errorf("listener %q: tls key: %w", l.Name, err)
			}
			if l.TLS.RequireClientCert {
				if l.TLS.ClientCAPath == "" {
					return fmt.Errorf("listener %q: tls require_client_cert but client_ca_path empty", l.Name)
				}
				if err := fileExists(l.TLS.ClientCAPath); err != nil {
					return fmt.Errorf("listener %q: tls client_ca_path: %w", l.Name, err)
				}
			}
			// Additional named certs (if any)
			for _, ce := range l.TLS.Certs {
				if err := fileExists(ce.CertPath); err != nil {
					return fmt.Errorf("listener %q: tls certs cert: %w", l.Name, err)
				}
				if err := fileExists(ce.KeyPath); err != nil {
					return fmt.Errorf("listener %q: tls certs key: %w", l.Name, err)
				}
			}
		}
		...
		if len(l.FilterChains) == 0 {
			return fmt.Errorf("listener %q: no filter_chains", l.Name)
		}
		for _, fc := range l.FilterChains {
			if len(fc.Filters) == 0 {
				return fmt.Errorf("listener %q: filter chain %q has no filters", l.Name, fc.Name)
			}

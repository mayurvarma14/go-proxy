...
func RenderProm() string {
    ...
    for k := range snap { keys = append(keys, k) }
    sort.Strings(keys)
    ...
    for _, k := range keys {
        ...
        if m := reClusterInflight.FindStringSubmatch(k); m != nil {
            ...
            if !emittedType[name] { b.WriteString("# TYPE "+name+" "+typeHints[name]+"\n"); emittedType[name]=true }
            b.WriteString(name)
        ...
        if m := reDownReq.FindStringSubmatch(k); m != nil {
            ...
            if !emittedType[name] { b.WriteString("# TYPE "+name+" counter\n"); emittedType[name]=true }
            b.WriteString(name)
        ...
        if m := reHistBucket.FindStringSubmatch(k); m != nil {
            ...
            le := m[3]
            name := base + "_bucket"
            if !emittedType[name] { b.WriteString("# TYPE "+name+" histogram\n"); emittedType[name]=true }
            b.WriteString(name)
            b.WriteString("{cluster=\"")
            b.WriteString(esc(cluster))
            b.WriteString("\",le=\"")
            if le == "Inf" { b.WriteString("+Inf") } else { b.WriteString(esc(le)) }
            b.WriteString("\"}")
            b.WriteString(" ")
            b.WriteString(intToString(v))
            b.WriteString("\n")

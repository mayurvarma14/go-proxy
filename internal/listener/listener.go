	...
	if spec.TLS != nil {
		...
		if len(spec.TLS.Certs) > 0 {
			...
			for _, ce := range spec.TLS.Certs {
				...
				if err != nil {
					return nil, fmt.Errorf("load tls keypair (extra): %w", err)
				}
			...
			tconf.GetCertificate = func(hello *tls.ClientHelloInfo) (*tls.Certificate, error) {
				...
				if c := certMap[strings.ToLower(hello.ServerName)]; c != nil {
					return c, nil
				}
				return def, nil
			}
		...
		if spec.TLS.RequireClientCert {
			...
			if err != nil {
				return nil, fmt.Errorf("tls: read client_ca_path: %w", err)
			}

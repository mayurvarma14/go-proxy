...
func epSet(eps []Endpoint) map[string]struct{} {
    ...
    for _, e := range eps { m[e.Addr] = struct{}{} }
    return m
}
...
func TestApplyEndpointSet_AddAndRemove(t *testing.T) {
    ...
    if _, ok := got["127.0.0.1:1"]; !ok { t.Fatalf("missing endpoint 127.0.0.1:1 after add") }
    if _, ok := got["127.0.0.1:2"]; !ok { t.Fatalf("missing endpoint 127.0.0.1:2 after add") }
    // State entries must exist for new endpoints
    c.mu.RLock()
    if _, ok := c.state["127.0.0.1:2"]; !ok { c.mu.RUnlock(); t.Fatalf("state not created for new endpoint") }
    c.mu.RUnlock()
...
func TestDebugSnapshot_Basic(t *testing.T) {
    ...
    if len(snap) != 1 { t.Fatalf("want 1 cluster in snapshot, got %d", len(snap)) }
    cd := snap[0]
    if cd.Name != "c1" { t.Fatalf("want name c1, got %q", cd.Name) }
    if cd.LBPolicy == "" { t.Fatalf("expected lb policy set") }

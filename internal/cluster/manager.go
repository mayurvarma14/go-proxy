...
func NewManager(specs []config.ClusterSpec) (*Manager, error) {
	...
	for _, s := range specs {
		...
		if s.HealthCheck != nil {
			...
			if hc.IntervalMillis <= 0 {
				hc.IntervalMillis = 2000
			}
			if hc.TimeoutMillis <= 0 {
				hc.TimeoutMillis = 1000
			}
			if hc.HTTPPath == "" {
				hc.HTTPPath = "/"
			}
...
func (c *Cluster) isAvailable(addr string, now time.Time) bool {
	...
	if st.activeUnhealthy {
		return false
	}
	if now.Before(st.unhealthyUntil) {
		return false
	}
	return true
}
...
func (c *Cluster) resolveOnce(ctx context.Context, seed, host, port string) {
	...
	for addr := range c.staticSeeds {
		effective[addr] = struct{}{}
	}
...
func (c *Cluster) applyEndpointSet(newSet map[string]struct{}) {
	...
	for addr := range newSet {
		next = append(next, Endpoint{Addr: addr})
	}
	c.Endpoints = next

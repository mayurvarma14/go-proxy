...
func NewUpstreamProxy(cm *cluster.Manager, clusters []config.ClusterSpec) UpstreamProxy {
	...
	for _, cs := range clusters {
		if cs.UpstreamTLS != nil && cs.UpstreamTLS.Enabled {
			...
			if cs.UpstreamTLS.ServerName != "" {
				tlsConf.ServerName = cs.UpstreamTLS.ServerName
			}
			tr2 := &http.Transport{
				Proxy:              http.ProxyFromEnvironment,
				MaxIdleConns:       100,
		...
		if cs.RetryPolicy != nil {
			if cs.RetryPolicy.RequestTimeoutMs > 0 {
				rc.requestTimeout = time.Duration(cs.RetryPolicy.RequestTimeoutMs) * time.Millisecond
			}
			if cs.RetryPolicy.PerTryTimeoutMs > 0 {
				rc.perTryTimeout = time.Duration(cs.RetryPolicy.PerTryTimeoutMs) * time.Millisecond
			}
			if cs.RetryPolicy.MaxRetries > 0 {
				rc.maxRetries = cs.RetryPolicy.MaxRetries
			}
			// default idempotentOnly=true even if field not set
			if !cs.RetryPolicy.IdempotentOnly {
				rc.idempotentOnly = false
			}
			if cs.RetryPolicy.BackoffBaseMs > 0 {
				rc.backoffBase = time.Duration(cs.RetryPolicy.BackoffBaseMs) * time.Millisecond
			}
			if cs.RetryPolicy.BackoffMaxMs > 0 {
				rc.backoffMax = time.Duration(cs.RetryPolicy.BackoffMaxMs) * time.Millisecond
			}
		}
		up.retry[cs.Name] = rc
...
func (u UpstreamProxy) OnRequest(rc *RequestCtx) error {
	...
	if rc.RetryOverride != nil {
		...
		if o.requestTimeout > 0 {
			pol.requestTimeout = o.requestTimeout
		}
		if o.perTryTimeout > 0 {
			pol.perTryTimeout = o.perTryTimeout
		}
		if o.maxRetries > 0 {
			pol.maxRetries = o.maxRetries
		}
		if o.backoffBase > 0 {
			pol.backoffBase = o.backoffBase
		}
		if o.backoffMax > 0 {
			pol.backoffMax = o.backoffMax
		}
	}
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		if rc.RewritePath != "" {
			fwdPath = rc.RewritePath
		}
		scheme := "http"
		if u.tlsEnabled[rc.UpstreamCluster] {
			scheme = "https"
		}
		...
		cli := u.cli
		if ccli, ok := u.clients[rc.UpstreamCluster]; ok {
			cli = ccli
		}

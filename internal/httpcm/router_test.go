...
func TestRouter_StripPrefix(t *testing.T) {
    ...
    rc := newRC("/svc/foo")
    _ = r.OnRequest(rc)
    if rc.UpstreamCluster != "svc" { t.Fatalf("want cluster svc, got %q", rc.UpstreamCluster) }
    if rc.RewritePath != "/foo" { t.Fatalf("want rewrite /foo, got %q", rc.RewritePath) }
}
...
func TestRouter_CatchAll(t *testing.T) {
    ...
    rc := newRC("/anything")
    _ = r.OnRequest(rc)
    if rc.UpstreamCluster != "default" { t.Fatalf("want cluster default, got %q", rc.UpstreamCluster) }
}

...
func NewRouteOptionsFilter(routes []config.RouteRule) RouteOptionsFilter {
    ...
    for i, rr := range routes {
        if rr.RetryPolicy != nil {
            ...
            if rr.RetryPolicy.RequestTimeoutMs > 0 { rc.requestTimeout = toDurMs(rr.RetryPolicy.RequestTimeoutMs) }
            if rr.RetryPolicy.PerTryTimeoutMs > 0 { rc.perTryTimeout  = toDurMs(rr.RetryPolicy.PerTryTimeoutMs) }
            if rr.RetryPolicy.MaxRetries > 0 { rc.maxRetries = rr.RetryPolicy.MaxRetries }
            if rr.RetryPolicy.BackoffBaseMs > 0 { rc.backoffBase = toDurMs(rr.RetryPolicy.BackoffBaseMs) }
            if rr.RetryPolicy.BackoffMaxMs > 0 { rc.backoffMax  = toDurMs(rr.RetryPolicy.BackoffMaxMs) }
            f.retry[i] = rc
        }
        if rr.HostRewrite != "" { f.hostRewrite[i] = rr.HostRewrite }
        if len(rr.SetHeaders) > 0 { f.hdrSet[i] = rr.SetHeaders }
        if len(rr.RemoveHeaders) > 0 {
            m := map[string]struct{}{}
            for _, k := range rr.RemoveHeaders { m[strings.ToLower(k)] = struct{}{} }
            f.hdrDrop[i] = m
        }
    }
    return f
